...
/*  66 */           d1 = -d1;
/*     */         }
/*  68 */         for (int j = i; j < this.m; j++) {
/*  69 */           this.QR[j][i] /= d1;
/*     */         }
/*  71 */         this.QR[i][i] += 1.0D;
/*     */ 
/*  74 */         for (int j = i + 1; j < this.n; j++) {
/*  75 */           double d2 = 0.0D;
/*  76 */           for (int k = i; k < this.m; k++) {
/*  77 */             d2 += this.QR[k][i] * this.QR[k][j];
/*     */           }
/*  79 */           d2 = -d2 / this.QR[i][i];
/*  80 */           for (int k = i; k < this.m; k++) {
...
/*     */         }
/* 199 */         d = -d / this.QR[j][j];
/* 200 */         for (int i2 = j; i2 < this.m; i2++) {
/* 201 */           arrayOfDouble[i2][k] += d * this.QR[i2][j];
/*     */         }
/*     */       }
/*     */     }
/*     */ 
/* 206 */     for (int j = this.n - 1; j >= 0; j--) {
/* 207 */       for (k = 0; k < i; k++) {
